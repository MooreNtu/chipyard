...
import saturn.common.{VectorParams} //saturn
...
class WithSystemModifications extends Config((site, here, up) => {
  ...
  case BootROMLocated(x) => up(BootROMLocated(x), site).map { p =>
    ...
    p.copy(hang = 0x10000, contentFileName = s"./fpga/src/main/resources/zcu102/sdboot/build/sdboot.bin")// frank changed size
  }
  case ExtMem => up(ExtMem, site).map(x => x.copy(master = x.master.copy(size = site(ZCU102DDRSize))))//change to AXI
...
class RocketSaturnZCU102Config extends Config(
